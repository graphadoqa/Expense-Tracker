...
function moreHTML(){
 ...
 <button class="menu" id="makeInvoice"><span>▤</span><div><strong>Make Invoice / Quotation</strong><small>PDF</small></div></button>
 <button class="menu" id="backup"><span>↓</span><div><strong>Backup & Restore</strong><small>JSON</small></div></button>
 <button class="menu" id="csv"><span>▦</span><div><strong>Export Expenses</strong><small>CSV</small></div></button>
 <button class="menu" id="settings"><span>⚙</span><div><strong>Settings</strong><small>${state.settings.currency}</small></div></button>
 </div>`;
}
